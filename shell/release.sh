...
if [ -z "$tag_version" ]; then
    log "エラー: $tag_version tag_version"
    exit 1
fi
...
#-----------------------------
log_mini_title "apply config_map"
#-----------------------------
exec_func "kubectl apply -f $config_map_yaml"
...
#-----------------------------
log_mini_title "mod deployment"
#-----------------------------
sed -ie "s/@tag@/$tag_version/g" $deploy_yaml
exec_func "cat $deploy_yaml"

#-----------------------------
log_mini_title "apply deployment"
#-----------------------------
exec_func "kubectl apply -f $deploy_yaml"

#-----------------------------
log_mini_title "kubectl rollout status"
#-----------------------------
exec_func "kubectl rollout status deployment $deploy_name -n $name_space"
